...
graph = cl.Classifier(init_graph, utils.unprotected_direction(x_unprotected_train, sensetive_directions), 
                        y_train, num_steps = 10000) # for fair algo
...
def sample_perturbation(data_point, regularizer = 5, learning_rate = 5e-2, num_steps = 200):
    ...
    for _ in range(num_steps):
        with tf.GradientTape() as g:
            g.watch(x_fair)
            prob = graph(utils.unprotected_direction(x_fair, sensetive_directions))
    ...
    for _ in range(num_steps):
        with tf.GradientTape() as g:
            g.watch(x_base)
            prob = graph(utils.unprotected_direction(x_base, sensetive_directions))
            perturb = x_base-x_start
            loss = utils.EntropyLoss(y, prob) - regularizer * tf.norm(perturb)**2

        gradient = g.gradient(loss, x_base)
        x_base = x_base + learning_rate * gradient #utils.protected_direction(gradient, sensetive_directions)

    #print(utils.unprotected_direction(x - x_start, sensetive_directions))
    return_loss = utils.EntropyLoss(y, graph(utils.unprotected_direction(x_fair, sensetive_directions)))\
         / utils.EntropyLoss(y, graph(utils.unprotected_direction(x_base, sensetive_directions)))
