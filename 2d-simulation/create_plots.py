...
div_norm = colors.DivergingNorm(vmin=0.75, vcenter=1.25, vmax=1.5)
...
fig, (ax0, ax1, ax2) = plt.subplots(nrows = 1, ncols = 3, figsize = [15, 4])
b = np.load('data/test_stat_0.npy')
im0 = ax0.pcolormesh(T1, T2, b, shading='gouraud', norm = div_norm, cmap = 'seismic')
...
fig.colorbar(im0, ax=ax0)
b = np.load('data/test_stat_10.npy')
im1 = ax1.pcolormesh(T1, T2, b, shading='gouraud', norm = div_norm, cmap = 'seismic')
...
fig.colorbar(im1, ax=ax1)
b = np.load('data/test_stat_20.npy')
im2 = ax2.pcolormesh(T1, T2, b, shading='gouraud', norm = div_norm, cmap = 'seismic')
ax2.set_title('Rotation $20^O$')
ax2.set_xlabel('$\\theta_1$')
ax2.set_ylabel('$\\theta_2$')
fig.colorbar(im2, ax=ax2)