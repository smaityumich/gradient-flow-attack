...
def get_consistency(X, l_pred, tf_X, gender_idx = 39, race_idx = 40, relationship_idx = [33, 34, 35, 36, 37, 38], husband_idx = 33, wife_idx = 38):
    '''
...
#    if K_protected == protected_directions.shape[1]:
#        dist_f = explore_dist(protected_directions)
...
#        dist_f = fair_dist(proj_compl, 0.)
    ...
    if Sigma_fair is None:
        ...
        dist_f = explore_dist(Sigma_fair)
        ...
        for it in range(epoch):
            ...
            if it > fair_start:
                ...
                if (not baseline_saved) and (fair_start>0):
                    ...
                    weights = [x.eval() for x in variables.values()]
                    np.save('./results/' + tf_prefix + '_' + tb_long + '_' + 'baseline-weights' + '_' + str(post_idx), weights)
...
#                        sess.run(reset_full_adv_weights)
        ...
        try:
            ...
            pass
        np.save('./results/' + tf_prefix + '_' + tb_long + '_' + 'fair-weights' + '_' + str(post_idx), weights)
...
#         except:
#             pass
#         np.save('./results/' + tf_prefix + '_' + tb_long + '_' + '%s-weights' % method + '_' + str(post_idx), weights)
